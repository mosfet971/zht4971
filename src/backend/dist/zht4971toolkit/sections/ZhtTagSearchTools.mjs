...
class ZhtTagSearchTools extends TagSearchToolkit {
    constructor(embeddingsJsonPath, dbDirPath, mk) {
        ...
        let setAllTagsEmbeddingsFunction = (newValue) => {
            database.setEntity(dbDirPath, mk, entityTypeForTagSearchSystemItem, "tagsTable", { value: newValue });
        };
        ...
        let setIndexListFunction = (key, valueList) => {
            ...
            database.setEntity(dbDirPath, mk, entityTypeForTagSearchSystemItem, listId, { value: valueList });
        };
        let getIndexListFunction = (key) => {
            let listId = stringToHash(key, mk);
            let value = [];
            try {
                value = (database.getEntity(dbDirPath, mk, entityTypeForTagSearchSystemItem, listId)).value;
            }
            catch (error) {
                value = [];
            }
        ...
        super(embeddingsJsonPath, getAllTagsEmbeddingsFunction, setAllTagsEmbeddingsFunction, getIndexListFunction, setIndexListFunction);
    }
}
