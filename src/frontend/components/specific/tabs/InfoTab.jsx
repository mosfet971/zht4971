...
zht4971 является приложением для хранения текста и файлов. Базовая единица информации в приложении - запись. Записи представляют собой объекты, содержащие основной текст (в котором могут быть добавлены файлы), название, список псевдонимов, а также ряд других параметров. Основная идея программы заключается в создании большого набора текстовых описаний различных вещей с прикрепленными к ним файлами (записей), связанных между собой ссылками, с предоставлением возможности осуществления прямого доступа к любой записи при помощи поиска и списков записей. Конкретный вариант взаимодействия пользователя с программой может варьироваться в широких пределах.
...
Для связывания нескольких записей можно использовать следующий метод: 
1. Создается запись x
1. В записи x составляется список ссылок на каждую из связываемых записей
1. В конец каждой из связываемых записей добавляется конструкция вида ![[name]], где name - название записи x
`;
